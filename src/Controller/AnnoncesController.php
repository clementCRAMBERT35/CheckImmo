...
class AnnoncesController extends AbstractController
{
    ...
    public function new(Request $request, AnnoncesRepository $annoncesRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $annonce->setAnnonceWithURl($form->get('url')->getData());

                if ($annoncesRepository->existeDeja($annonce)) {
                    $annonce = $annoncesRepository->findOneBy(['Reference' => $annonce->getReference()]);
                }

                $annonce->addUser($this->getUser());
